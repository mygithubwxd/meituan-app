<template>
  <div class="header">
    ...
    <transition name="bulletin-detail">
      <!-- v-show指令根据条件的真假进行渲染（dispaly:none） -->
...
export default {
  ...
  // 计算属性
  computed:{
    // 利用计算属性，直接使用v-bind绑定样式style(字符串拼接的形式)，
    // css中background-size和background-position不会生效
    head_pic_url(){
